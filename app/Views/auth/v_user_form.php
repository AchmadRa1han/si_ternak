<div class="container mx-auto px-6 py-10 bg-[#faf7f2] min-h-screen">
    ...
    <div class="max-w-4xl mx-auto flex justify-between items-center mb-8 border-b border-stone-200 pb-4"
        ...
        <div>
            ...
            <p class="text-stone-500 text-sm mt-1">Lengkapi informasi detail akun pengguna sistem</p>
        </div>
    ...
    <div class="max-w-4xl mx-auto bg-white rounded-2xl shadow-sm border border-stone-100 overflow-hidden"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="nama_lengkap" id="nama_lengkap" 
                           class="w-full px-4 py-3 bg-stone-50 border border-stone-200 rounded-2xl focus:ring-2 focus:ring-[#a27b5c] focus:border-transparent transition-all outline-none text-stone-800 placeholder-stone-400 font-medium"
                ...
                <!-- Username -->
                <div class="space-y-2">
                    <label for="username" class="block text-sm font-bold text-[#1a120b] tracking-tight">Username</label>
                ...
                <div class="md:col-span-2 pt-4">
                    <h3 class="text-xs font-bold text-[#a27b5c] uppercase tracking-widest mb-4 flex items-center">
                        <span class="w-8 h-[1px] bg-[#a27b5c] mr-3"></span> Detail Pengguna
                    </h3>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <select name="role" id="role" 
                            class="w-full px-4 py-3 bg-stone-50 border border-stone-200 rounded-2xl focus:ring-2 focus:ring-[#a27b5c] focus:border-transparent transition-all outline-none bg-white text-stone-800 font-medium" required>
                        <?php $role = isset($user) ? $user->role : set_value('role'); ?>
                        <option value="admin" <?= ($role == 'admin') ? 'selected' : ''; ?>>Admin</option>
                        <option value="operator" <?= ($role == 'operator') ? 'selected' : ''; ?>>Operator</option>
                        <option value="penandatangan" <?= ($role == 'penandatangan') ? 'selected' : ''; ?>>Penandatangan</option>
        ...
        <div class="px-8 py-6 bg-stone-50 border-t border-stone-100 flex items-center justify-end gap-4">
            ...
            <button type="submit" class="px-10 py-2.5 bg-[#1a120b] hover:bg-[#a27b5c] text-white rounded-2xl font-bold transition-all duration-300 shadow-sm text-sm tracking-wide">
                Simpan Data
            </button>
        </div>
        <?= form_close(); ?>
